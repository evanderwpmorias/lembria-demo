...
import {stories as storiesModel, handleGet, handlePost, handleError } from '#server/mongoSchema'
...
export default defineEventHandler(async (event) => {
    ...
    switch (event.node.req.method) {
        case 'GET':
            return await handleGet(storiesModel, event)
        case 'POST':
            return await handlePost(storiesModel, event)
        default:

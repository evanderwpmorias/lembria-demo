    ...
    import {users as usersModel, handleGetById, handleDelete, handlePut, handlePatch, handleError } from '#server/mongoSchema'
   ...
   // import { authorizeRequest } from '#server/helpers/0.0.1/auth-middleware'
        ...
        switch (event.node.req.method) {
            case 'GET':
                return await handleGetById(usersModel, event)
            case 'DELETE':
                return await handleDelete(usersModel, event)
            case 'PUT':
                return await handlePut(usersModel, event)
            case 'PATCH':
                return await handlePatch(usersModel, event)
            default:

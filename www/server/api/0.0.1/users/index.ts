...
import {users as usersModel, handleGet, handlePost, handleError } from '#server/mongoSchema'
...
export default defineEventHandler(async (event) => {
    ...
    switch (event.node.req.method) {
        case 'GET':
            return await handleGet(usersModel, event)
        case 'POST':
            return await handlePost(usersModel, event)
        default:

    ...
    import {families as familiesModel, handleGetById, handleDelete, handlePut, handlePatch, handleError } from '#server/mongoSchema'
   ...
   // import { authorizeRequest } from '#server/helpers/0.0.1/auth-middleware'
        ...
        switch (event.node.req.method) {
            case 'GET':
                return await handleGetById(familiesModel, event)
            case 'DELETE':
                return await handleDelete(familiesModel, event)
            case 'PUT':
                return await handlePut(familiesModel, event)
            case 'PATCH':
                return await handlePatch(familiesModel, event)
            default:

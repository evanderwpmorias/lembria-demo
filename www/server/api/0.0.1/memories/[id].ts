    ...
    import {memories as memoriesModel, handleGetById, handleDelete, handlePut, handlePatch, handleError } from '#server/mongoSchema'
   ...
   // import { authorizeRequest } from '#server/helpers/0.0.1/auth-middleware'
        ...
        switch (event.node.req.method) {
            case 'GET':
                return await handleGetById(memoriesModel, event)
            case 'DELETE':
                return await handleDelete(memoriesModel, event)
            case 'PUT':
                return await handlePut(memoriesModel, event)
            case 'PATCH':
                return await handlePatch(memoriesModel, event)
            default:

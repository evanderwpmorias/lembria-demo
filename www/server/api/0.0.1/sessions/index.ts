...
import {sessions as sessionsModel, handleGet, handlePost, handleError } from '#server/mongoSchema'
...
export default defineEventHandler(async (event) => {
    ...
    switch (event.node.req.method) {
        case 'GET':
            return await handleGet(sessionsModel, event)
        case 'POST':
            return await handlePost(sessionsModel, event)
        default:

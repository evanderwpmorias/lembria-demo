    ...
    import {messages as messagesModel, handleGetById, handleDelete, handlePut, handlePatch, handleError } from '#server/mongoSchema'
   ...
   // import { authorizeRequest } from '#server/helpers/0.0.1/auth-middleware'
        ...
        switch (event.node.req.method) {
            case 'GET':
                return await handleGetById(messagesModel, event)
            case 'DELETE':
                return await handleDelete(messagesModel, event)
            case 'PUT':
                return await handlePut(messagesModel, event)
            case 'PATCH':
                return await handlePatch(messagesModel, event)
            default:

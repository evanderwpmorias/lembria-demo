...
From the `services/studio` directory:

```bash
docker build -t studio-bidi-demo .
docker run --rm -p 8080:8080 \
  -e GOOGLE_API_KEY=your_api_key_here \
  -e GOOGLE_GENAI_USE_VERTEXAI=FALSE \
  -e DEMO_AGENT_MODEL=gemini-2.5-flash-native-audio-preview-12-2025 \
  studio-bidi-demo
```

The container listens on the Cloud Run-compatible `PORT` environment variable and defaults to `8080`.

### Deploy To Cloud Run

The repo includes a PowerShell deployment script at `scripts/deploy-cloud-run.ps1`. It:

- Ensures an Artifact Registry Docker repository exists
- Builds the image with Google Cloud Build
- Deploys the image to Cloud Run

Example:

```powershell
pwsh ./scripts/deploy-cloud-run.ps1 \
  -ProjectId your-gcp-project \
  -Region us-central1 \
  -ServiceName studio-bidi-demo \
...
- Google Cloud SDK with `gcloud` authenticated against your project
- Cloud Build, Artifact Registry, and Cloud Run APIs enabled

If you prefer the raw commands instead of the script:
...
gcloud builds submit \
  --tag us-central1-docker.pkg.dev/your-gcp-project/cloud-run-images/studio \
  .

gcloud run deploy studio-bidi-demo \
  --image us-central1-docker.pkg.dev/your-gcp-project/cloud-run-images/studio \
  --region us-central1 \
...
bidi-demo/
├── app/
│   ├── __init__.py               # Package marker for root-level imports
│   ├── reception_interviewer/    # Agent definition module
...
├── scripts/
│   └── deploy-cloud-run.ps1      # Cloud Run deployment helper
...
### Agent Definition (app/reception_interviewer/agent.py)
...
```python
from app.reception_interviewer.agent import agent
...
Set the model via `DEMO_AGENT_MODEL` in `.env` or modify `app/reception_interviewer/agent.py`.
